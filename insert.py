...
#             print("数据插入成功")
    ...
    data = (
        ...
        int(json_data.get("publication_year", 0)) if json_data.get("publication_year") is not None else None,  # 确保是 int 或 None
        json_data.get("publication_date"),  # 确保是 str
        json_data.get("type"),  # 确保是 str
        int(json_data.get("cited_by_count", 0)) if json_data.get("cited_by_count") is not None else None,  # 确保是 int 或 None
    ...
    # 插入数据的 SQL 语句
    sql = """
    INSERT INTO openalex_works (id, doi, title, display_name, publication_year, publication_date, type, cited_by_count, is_retracted, is_paratext, cited_by_api_url, abstract_inverted_index, language,grants,keywords)
...
#         插入insitutions
            ...
            for x in range(len10):
                ...
                data10 = (
                    ...
                    institution.get("image_thumbnail_url") if institution.get("image_thumbnail_url") is not None else None,
...
# 暂时不添加，速度太大，数据太庞杂，不如直接导入所有concepts后再两两插值
#     concepts_related_concepts先插入这个表然后把相关的concepts都插进来
#         related_concepts = json_data3.get("related_concepts")
#         print(related_concepts)
#         if(related_concepts is not None):
#             len3 = len(related_concepts)
#             for k in range(len3):

#                 data5 = (
#                     concept_id,
#                     related_concepts[k].get("id"),
#                     float(related_concepts[k].get("score")) if related_concepts[k].get("score") is not None else None,
#                 )
#                 sql5 = """
#                 INSERT INTO openalex_concepts_related_concepts (concept_id,related_concept_id,score)
#                 VALUES (%s, %s, %s)
#                 """
#                 try:
#                     # 执行 SQL 插入语句
...
#                 json_data6 = Concepts()[related_concepts[k].get("id")]
#                 data6 = (
#                     related_concepts[k].get("id"),
#                     json_data6.get("wikidata") if json_data6.get("wikidata") is not None else None,
#                     json_data6.get("display_name") if json_data6.get("display_name") is not None else None,
#                     int(json_data6.get("level")) if json_data6.get("level") is not None else None,
#                     json_data6.get("description") if json_data6.get("description") is not None else None,
#                     int(json_data6.get("works_count")) if json_data6.get("works_count") is not None else None,
#                     int(json_data6.get("cited_by_count")) if json_data6.get("cited_by_count") is not None else None,
#                     json_data6.get("image_url") if json_data6.get("image_url") is not None else None,
#                     json_data6.get("image_thumbnail_url") if json_data6.get("image_thumbnail_url") is not None else None,
#                     json_data6.get("works_api_url") if json_data6.get("works_api_url") is not None else None,
#                     json_data6.get("updated_date") if json_data6.get("updated_date") is not None else None,
#                 )
#                 sql6 = """
#                 INSERT INTO openalex_concepts (id, wikidata,display_name,level,description,works_count,cited_by_count,image_url,image_thumbnail_url,works_api_url,updated_date)
#                 VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s)
